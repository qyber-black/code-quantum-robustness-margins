...
def test_tv_bracket_is_ordered(case):
    """r0 <= m_ub; a found violation certifies the upper bound."""
    problem, controllers = case
    c = controllers[0]
    dt = c["tf"] / c["tau"]
    tau = c["tau"]
    ...
    # trajectory exploited time variation; either way m_ub <= 2 M by search.
    assert br.m_ub <= 2.0 * M * (1 + 1e-12)
