...
#: Upper case and digits both matter: --FT and --n2-starts are flags.
...
def _recipes(text: str) -> list[str]:
    """The Makefile split into one chunk per target.

    A target header starts at column 0 and contains a colon; everything up to
    the next such line is that target's recipe. Crude, but enough to ask
    whether a single recipe repeats an argument list.
    """
    chunks, cur = [], []
...
def test_each_driver_recipe_references_its_flags_variable():
    ...
    # Presence anywhere is too weak once a driver's flags are used by more
    # than one target: collapsing one recipe's index onto another leaves the
    # index still referenced elsewhere, while that recipe quietly writes the
    # wrong file. Within a single recipe the references must be distinct --
    # two invocations passing the same argument list means one of the
    # driver's outputs is never regenerated. Across recipes a repeat is only
    # redundant work, not a lost artefact, so it is allowed.
    for recipe in _recipes(MAKEFILE):
        for script, runs in DRIVER_RUNS.items():
            stem = script[:-3]
            want = {f"FLAGS_{stem}"} | {
                f"FLAGS_{stem}_{i}" for i in range(1, len(runs))
            }
            seen = [
                v
                for sc, args in _invocations(recipe)
                if sc == script
                for v in _flags_vars(args)
                if v in want
            ]
            dupes = sorted({v for v in seen if seen.count(v) > 1})
            assert not dupes, (
                f"{script}: one recipe references {dupes} more than once; "
                "two invocations with the same argument list leave one of the "
                "driver's outputs unregenerated."
            )
...
def test_the_guards_catch_a_reintroduced_divergence():
    ...
    and a guard that has not been shown to fail is not a guard.
    """
    import pytest
