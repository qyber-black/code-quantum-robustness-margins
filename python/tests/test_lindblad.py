...
def test_unitary_superop_consistency():
    """A dissipation-free generator propagates to exactly the superoperator
    of the closed-system unitary."""
...
def test_frechet_matches_fd():
    """The block Frechet derivative matches a central difference on a
    diagonalisable generator."""
...
def test_diamond_norm_analytic_cases():
    """Three cases with closed forms: the identity channel is 1, a small
    rotation's distance to it is 2 sin(theta), and the depolarising
    channel's distance to the identity is 3p/2 on a qubit."""
...
def test_diamond_norm_scaling_and_conservatism():
    """The norm is homogeneous, and the certified value never falls below
    the solver's raw optimum -- a robustness constant must over-estimate."""
    U = expm(-1j * 0.4 * SZ)
...
def test_common_rate_diamond_norms_are_2n():
    ...
    for n in (1, 2):
        for op in (Z, SM):
            ...
            # One-sided, because the two directions are not equally
            # serious. Falling below 2n would make a certificate unsound,
            # so that bound stays at roundoff. Sitting above it only costs
            # conservatism, and how far above depends on the SDP solver's
            # own accuracy: the same generators land within 7e-6 of 2n on
            # one solver build and 1.5e-4 on another. The certified
            # deviation actually observed is recorded per generator by
            # run_dnorm_certificates.py, and the paper quotes it from
            # there; a tolerance here that tracked one build's accuracy
            # would fail on the next.
            assert dn.value_certified >= 2 * n - 1e-9
            assert dn.value_certified <= 2 * n * (1 + 1e-3)


def test_the_sdp_solver_is_named_not_resolved():
    """The diamond-norm solver is the package's choice, not cvxpy's.

    cvxpy routes complex SDPs to SCS even though CLARABEL ranks above it in
    cvxpy's own preference order, and SCS is up to five orders of magnitude
    looser on these generators while warning that its solution may be
    inaccurate. Worse, leaving the choice to the resolver made published
    open-system numbers differ by up to 4e-5 relative across a virtualenv
    rebuild whose package versions were identical, so pinning versions does
    not substitute for naming the solver.

    Soundness never depended on it: the repair step returns a rigorous
    upper bound whatever the solver reports. Tightness and reproducibility
    did.
    """
    import numpy as np
    ...
    if lb.DEFAULT_SDP_SOLVER not in cp.installed_solvers():
        pytest.skip(f"{lb.DEFAULT_SDP_SOLVER} not installed")

    Z = np.diag([1.0, -1.0]).astype(complex)
    I2 = np.eye(2, dtype=complex)
    G = lb.dissipator(np.kron(Z, I2)) + lb.dissipator(np.kron(I2, Z))

    dn = lb.diamond_norm(G)
    # An explicit "" hands the choice back to cvxpy, which is what the
    # package used to do implicitly; keeping that path exercised means the
    # override still works when a user has a solver we do not name.
    ...
    assert dn.solver == lb.DEFAULT_SDP_SOLVER, (
        ...
        f"{lb.DEFAULT_SDP_SOLVER} so the result does not depend on which "
        "cvxpy is installed"
    )
    # The repair gap is the symptom a silent fallback would show: SCS
    # returns 2e-4 here where the named solver stays near roundoff.
