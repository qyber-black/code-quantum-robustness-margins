...
def test_traceless_is_traceless_and_projection(structures):
    """Centring kills the trace and is idempotent, so applying it twice is
    applying it once -- what lets every gauge assume centred structures."""
    H = structures[0][0]
    Hb = traceless(H)
...
def test_interval_grams_symmetric_psd(structures):
    """The per-interval Gram matrices are symmetric and positive
    semidefinite in both the raw and the centred-normalised form."""
    for grams in (
...
def test_positive_homogeneity(structures):
    """The gauge is absolutely homogeneous: scaling the direction scales the
    constant, which is what makes a radial certificate meaningful."""
...
def test_alpha_cs_bounds_sphere_samples(structures):
    """The Cauchy-Schwarz bound alpha really bounds the gauge: 200 sampled
    unit directions must all fall under it."""
    gauge = PathGauge(grams=interval_grams(structures), dt=0.3)
...
def test_margin_from_guard():
    """A zero constant is unbounded robustness, not a division by zero."""
    assert margin_from(0.5, 0.0) == float("inf")
    assert margin_from(0.5, 2.0) == 0.25


def test_angle_budget_clamps_and_signs():
    """The angular budget is arccos FT at unit fidelity, clamps a fidelity
    nudged above 1 by rounding, and is exactly zero at the threshold."""
...
def test_refine_structure(structures):
    """Refining by q repeats each interval q times in place, so the refined
    lists still line up with the original grid."""
