...
def test_classes_and_safety_on_ensemble():
    ...
    for tag in ("H0", "H1", "H2"):
        ...
        mb_i = berberich.margin(
            H_list, dH, dt, 0.999, nominal_error=c["error"], uncertainty="independent"
        )
        mb_s = berberich.margin(
            H_list, dH, dt, 0.999, nominal_error=c["error"], uncertainty="systematic"
        )
        ...
        assert gate_fidelity(U, problem["Uf"]) >= 0.999 - 1e-12


def test_vacuous_when_budget_exhausted():
    mb = berberich.margin([0.3 * SZ], [SX], 1.0, 0.999, nominal_error=0.01)
