...
def fidelity_cross_check(H_list: HList, dt: float, Uf: Array) -> tuple:
    ...
    for H in H_list:
        ...
        U2 = _segment_propagator(lam, V, dt) @ U2
...
def check_metric_triangle(N: int, n_triples: int = 200, seed: int = 0) -> CheckReport:
    ...
    theta = lambda U, V: np.arccos(min(1.0, abs(np.trace(U.conj().T @ V)) / N))
    worst = np.inf
    arg = None
    for i in range(n_triples):
        ...
        if slack < worst:
            worst, arg = slack, i
    return CheckReport(
        "metric_triangle", n_triples, float(worst), tol=1e-7, argmin=arg
...
def check_absorption(FT: float, N: int, n: int = 200, seed: int = 0) -> CheckReport:
    ...
    for i in range(n):
        ...
        if slack < worst:
            worst, arg = slack, i
    return CheckReport("absorption", n_done, float(worst), tol=1e-12, argmin=arg)
...
def check_lipschitz_pairs(
    ...
    for i in range(n):
        ...
        if slack < worst:
            worst, arg = slack, i
    return CheckReport("lipschitz_pairs", n_done, float(worst), tol=1e-12, argmin=arg)
...
def check_tv_slope(
    ...
    if n_done == 0:
        return CheckReport("tv_slope", 0, float("inf"), tol=1e-12)
    return CheckReport(
        "tv_slope",
        n_done,
        float(worst),
...
def check_fs_angle(
    ...
    for i in range(n):
        ...
        if slack < worst:
            worst, arg = slack, i
    return CheckReport("fs_angle", n, float(worst), tol=1e-9, argmin=arg)
