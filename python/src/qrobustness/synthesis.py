...
interval; see ``core._dU_dmu_exact``), so synthesis introduces no new
numerics.
...
def fidelity_and_control_gradient(
    ...
    for k in range(tau):
        ...
        eigs.append((lam, V))
        Pref.append(_segment_propagator(lam, V, dt) @ Pref[-1])
    Utot = Pref[tau]
    ...
    for k in range(tau - 1, -1, -1):
        lam, V = eigs[k]
        Suff[k] = Suff[k + 1] @ _segment_propagator(lam, V, dt)

    g = np.zeros_like(u)
    for k in range(tau):
        lam, V = eigs[k]
        for j in range(n_ctrl):
            dUk = _dU_dmu_exact(lam, V, H_ctrls[j], dt)
