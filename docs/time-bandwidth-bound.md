...
this package. The finite perturbation margin it implies can then be placed
...
1. Theorem 1 of the reference assumes `F_nom = 1` exactly and bounds the
   ...
   `absorption='angular'`). The additive absorption `F_T + eps_0` is
   strictly weaker than required and hence NOT conservative; it is
   selectable (`absorption='additive'`) only to reproduce numbers published
   with it. At the largest `eps_0 = 9.9e-5` in the set
...
3. The margin depends on the uncertainty class. The default
   ...
   certifies constant perturbations only. It is NOT a sup-norm
...
| `H2`      | 9.41e-3    | 3.14e-3      | 3.04 (2.40-4.19)      | 0.82    |
...
```bash
make paper-QRM-time-bandwidth ENGINE=python   # -> results/time-bandwidth-bound-python/
make paper-QRM-time-bandwidth ENGINE=matlab   # -> results/time-bandwidth-bound-matlab/
make paper-QRM-time-bandwidth ENGINE=octave   # -> results/time-bandwidth-bound-octave/
make test-parity ENGINE=matlab                # compare that engine against Python
```

Each engine writes three tables: the angular default, the additive
absorption, and the sup-norm trajectory class `M^K_tv`.
The three engines agree to about `1e-13` on the full 61-controller
table (`atol=1e-10`, `rtol=1e-8`, via
`scripts/compare_time_bandwidth_bound.py`). The drivers write identical CSV
columns, fixed by `CSV_HEADERS` in
...
The committed `kosut_comparison_*.csv` tables carry the `k_*` fields
(`k_w_unc`, `k_w_avg`, `k_w_dev`, `k_T`, `k_M`), and `make test-parity`
compares them across languages with `scripts/compare_time_bandwidth_bound.py`,
alongside the margin and sensitivity values. Unit tests
...
the angular option is queued, and the cross-language comparison
(`scripts/compare_time_bandwidth_bound.py`) therefore runs on the additive
CSVs.
