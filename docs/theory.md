...
fidelity a safe island beyond the first crossing cannot inflate `M`. The
...
an empirical upper bound, so `M_tv` is bracketed `[r_0, m_ub]` -- the
...
| `dnorm` | `diamond_norm` | SDP; primal value plus recorded duality gap enters `L` conservatively |
