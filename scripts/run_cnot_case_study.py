...
from _drivers import EYE2, PAULI_Z, cnot_model
...
def main() -> None:
    ...
    for ci, r in enumerate(ens):
        ...
        F0p = F_pro(0.0)
        if F0p > ft_pro:
            om = lb.open_margin(F_pro, L_gamma, ft_pro, margin_tol=1e-6)
            row["M_gamma"] = om.M_plus
            row["r0_gamma"] = (F0p - ft_pro) / L_gamma
            lo, hi = om.M_plus, max(10 * om.M_plus, 1e-6)
