...
JOBS = str(min(32, os.cpu_count() or 4))
...
def _with_jobs(table):
    ...
    not experiment, and the Makefile supplies its own from JOBS.
    """
...
PAPERS = {
    "xqrm": {
        ...
        "driver_runs": _with_jobs(DRIVER_RUNS),
        "trees": [
...
#: Columns that measure the machine rather than the science. Wall-clock
#: cannot reproduce -- the same driver on the same host varied by a factor
#: of two between runs purely with load -- so comparing it turns every
#: reproduction into a failure and hides the columns that matter. The
#: deterministic cost measure is the evaluation count beside it.
VOLATILE_COLUMNS = frozenset({"t_iter_s", "t_fs_s", "t_kosut_s", "t_synth_s"})
...
def compare_csv(a: Path, b: Path, rtol: float, atol: float, rep: Report):
    ...
    for i, (rowa, rowb) in enumerate(zip(ra, rb)):
        if len(rowa) != len(rowb):
            ...
            return
        for j, (x, y) in enumerate(zip(rowa, rowb)):
            if j in skip or x == y:
                continue
            try:
