...
from _drivers import PAULI_Z, base_parser, load_ensemble, true_crossing
...
def main() -> None:
    """Certified amplitude-damping and joint two-rate margins per controller."""
    ...
    # Both are common-rate local jump families, whose diamond norm is
    # the exactly proved 2n; the SDP would only reproduce it to solver
    # accuracy (cross-checked in run_dnorm_certificates.py).
    ...
    for ci, c in enumerate(controllers):
        dt = c["tf"] / c["tau"]
        L_z = lb.rate_lipschitz(dn_z.value, c["tf"])
        L_amp = lb.rate_lipschitz(dn_amp.value, c["tf"])
        H_list = [
        ...
        def F_pro(gz: float, ga: float, GH=GH, dt=dt) -> float:
        ...
        g_star = true_crossing(lambda g: F_pro(g, g), ft_pro, M_diag)
