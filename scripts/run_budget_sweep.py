...
def main() -> None:
    ...
    ap.add_argument("--structure", default="H1")
    ...
    j = {"H0": 0, "H1": 1, "H2": 2}[tag]
    r0 = uniform_margin(L[j], c["fid"], ft)
    rfs = fs_margin(dH, dt, c["fid"], ft).r_fs

    budgets = np.geomspace(0.5 * r0, 1.4 * KM, args.n_budgets)
    rows = []
    for bi, m in enumerate(budgets):
        ...
        for g, q in enumerate((1, 4, 16)):
            ...
            Fmin, _ = adversarial_fidelity(
                ...
                seed=args.seed + 100 * bi + g,
            )
            row[f"Fmin_x{q}"] = Fmin
        rows.append(row)
        print(
            f"budget {bi + 1}/{len(budgets)} m={m:.3e}: "
            + " ".join(f"x{q}={row[f'Fmin_x{q}']:.6f}" for q in (1, 4, 16)),
            flush=True,
        )
