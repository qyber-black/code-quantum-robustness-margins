...
results/paper-xqrm/figures/*.pdf, which "make sync-paper-xqrm" copies into
...
[r_0, m_ub] on M_tv for the probed controllers.
...
# Which driver produces which input; see gen_paper_xqrm_tables.py. A figure
# whose input is absent used to be skipped silently, leaving the previously
# generated PDF in place.
...
def fig_margins(res: Path, out: Path) -> None:
    ...
    # Adversarial brackets [r0, m_ub] on M_tv for the probed controllers.
    first = True
    for r in tvb:
        px = pos_of[int(r["controller"])]
        lo, hi = float(r["r0"]), float(r["m_ub"])
        ax.plot(
            [px, px],
            [lo, hi],
            color=GRAY,
...
def fig_directions(res: Path, out: Path, controller: int = 1) -> None:
    ...
    Mub = np.array([float(row[f"Mub_{n}"]) for n in dir_names])
    ...
    ax.set_yscale("log")
    ax.set_ylim(top=Mub.max() * 3.5)  # room for the legend above the data
    ax.set_xticks(x)
    ax.set_xticklabels([f"${l}$" for l in labels], rotation=60, fontsize=6)
...
def fig_single_qubit(res: Path, out: Path) -> None:
    ...
    for ax, row, F, name in zip(axes, sq, (F_amp, F_det), ("amplitude", "detuning")):
...
def fig_ratios(res: Path, out: Path) -> None:
    ...
    for i, (label, mk, rk, mkd, rkd) in enumerate(data):
...
def fig_validity(res: Path, out: Path) -> None:
    ...
    for tok in lines[-1].lstrip("# ").split():
        if "=" in tok:
            ...
            certs[k] = float(v)
    rows = [
        dict(zip(lines[0].split(","), l.split(",")))
        for l in lines[1:]
        if not l.startswith("#")
    ]
