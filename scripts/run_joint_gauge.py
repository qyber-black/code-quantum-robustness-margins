...
def main() -> None:
    ...
    sphere = mp.sphere_directions(3, 200, seed=0)
    ...
    for i, c in enumerate(controllers):
        ...
        _, L = mp.structure_constants(specs, dt, c["tau"], ft, N)
        L = np.asarray(L)
        surplus = c["fid"] - ft
        ...
        for d in diagonals:
            ...
            diag_gain.append(G.boundary_radius(d, surplus, ft, N) / sep)
        ...
        m_joint = budget / ell(np.ones(3))
        m_sep = budget / s_sep
        traj_gain = m_joint / m_sep

        r_gauge_est = min(
            G.boundary_radius(d, surplus, ft, N) for d in sphere
        )  # sampled ESTIMATE only
        r_gauge_cert = G.inradius_certified(surplus, ft, N)
                ...
                "ang_diag_gain": float(
                    np.median(
                        [
                            ...
                            / G.boundary_radius(d, surplus, ft, N)
                            for d in diagonals
                        ]
                    )
                ),
    ...
    out = args.out
    path = write_rows(out / f"joint_gauge_{ft:g}.csv", rows)
    for key in (
        "diag_gain_med",
        "inradius_gain_cert",
        "inradius_gain_est",
