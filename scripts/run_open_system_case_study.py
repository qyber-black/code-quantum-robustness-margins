...
def main() -> None:
    ...
    for ci, c in enumerate(controllers):
        ...
        lo, hi = M_gamma, max(10 * M_gamma, 1e-6)
        while F_pro(hi) >= ft_pro and hi < 1e3:
            lo, hi = hi, 10 * hi
        for _ in range(60):
            mid = 0.5 * (lo + hi)
            if F_pro(mid) >= ft_pro:
    ...
    out = args.out
    path = write_rows(out / f"open_margins_{args.FT:g}.csv", rows)
...
def write_coherent(args, problem, controllers, ft_pro, out) -> None:
    ...
    for ci, c in enumerate(controllers[: args.coherent_controllers]):
        ...
        def F_pro(mu: float) -> float:
            G = [g + float(mu) * d for g, d in zip(GH, Gd)]
