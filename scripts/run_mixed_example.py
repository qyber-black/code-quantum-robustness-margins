...
from _drivers import base_parser, load_ensemble
...
def main() -> None:
    ...
    L_gamma = 0.5 * c["tf"] * lb.diamond_norm(G_z).value_certified
    ...
    mu_max = 3.0 * surplus / L_mu * 50  # wide enough to catch the contour
    g_max = 2.5 * surplus / L_gamma
    ...
    angles = np.array([0.0, 22.5, 45.0, 67.5, 90.0]) * np.pi / 180.0
    rays = []
    for a in angles:
        ...
        om = lb.open_margin(F_ray, L_d, ft_pro, margin_tol=1e-6)
        rays.append(
            {
