...
and the uniform trajectory certificates at budget max(r_0, r_FS)
...
from _drivers import three_structure_specs
...
def main() -> None:
    ...
    for ci, c in enumerate(controllers):
        ...
        for j, tag in enumerate(STRUCTURES):
            ...
            M = float(
                iterative_margin(
                    fid_fn, L[j], ft, mu0=0.0, eta=1e-6, margin_tol=MARGIN_TOL
                ).M
            )
            record(
                scope,
