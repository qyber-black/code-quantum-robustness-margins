...
%   Peer of scripts/run_open_system_case_study.py.
    ...
    % 1e-8, matching the reference's MARGIN_TOL in the open drivers.
    margin_tol = 1e-6;
    if isempty(opt.root)
    ...
    for ci = 1:numel(controllers)
        c = controllers{ci};
        dt = c.tf / c.tau;
        % Per controller, not hoisted: tf is a per-row field, and the
        % shipped ensemble is uniformly tf=15 only by accident of how it
        % was synthesised.
...
function value = crossing(fn, threshold, lo)
    % Bisect the true threshold crossing above a certified margin.
    % Returns the LOWER end of the final bracket, a rate at which the
    % fidelity was evaluated and found to meet the threshold: a guarantee,
    % and a lower bound on the crossing, not the midpoint. Peer of
...
function G = dephasing_generator(n)
    % Sum of the single-qubit dephasing dissipators on n qubits.
    sz = [1, 0; 0, -1];
    G = 0;
    for q = 1:n
        V = 1;
...
function output = ternary(condition, yes, no)
    % Inline conditional, which the language does not provide.
    if condition; output = yes; else; output = no; end
end

function write_csv(path, headers, rows)
    % Write a numeric table in the shared CSV format (LF, 16 digits).
    fid = fopen(path, 'w');
