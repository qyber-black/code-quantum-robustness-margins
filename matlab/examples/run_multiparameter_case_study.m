...
%   caller.  The angular stepping rule is the default paper protocol.
    ...
    directions = [qrobustness.multiparam.axis_directions(3); ...
                  qrobustness.multiparam.diagonal_directions(3)];
    ...
    for j = 1:numel(names)
        ...
        headers{end+1} = ['Mub_' names{j}]; %#ok<AGROW>
    ...
    for ci = 1:numel(controllers)
        ...
        for di = 1:size(directions, 1)
            d = directions(di, :).';
            kwargs = {'margin_tol', 1e-8, 'return_diagnostics', true, ...
