...
%   depend on the Python implementation being right.
    ...
    % Seeded so a failure is reproducible, as the Python peer's
    % test_lindblad.py does; the property holds for every R, so the seed
    % cannot decide the outcome, only which R exhibits a regression.
    ...
    assert(norm(qrobustness.lindblad.channel({Z, Z}, 0.5) - eye(size(G, 1)), ...
                'fro') < 1e-12);
    % An empty interval list has no meaning.
    assert_error(@() qrobustness.lindblad.channel({}, 0.1), ...
